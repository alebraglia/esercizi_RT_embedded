...
struct supermercato_t
{
    ...
    pthread_cond_t cassa[N], finito[N], arrivo_cliente[N];
    int coda_cassa[N], oggetti_cassa[N], cassa_finito[N], cliente_allo_sportello[N];
} supermercato;
...
void init_supermercato(struct supermercato_t *s)
{
    ...
    {
        pthread_cond_init(&s->cassa[i], &c_attr);
        pthread_cond_init(&s->finito[i], &c_attr);
        pthread_cond_init(&s->arrivo_cliente[i], &c_attr);
        s->coda_cassa[i] = 0;
        s->oggetti_cassa[i] = 0;
        s->cassa_finito[i] = 0;
        s->cliente_allo_sportello[i] = 0;
    }
...
void cliente_pagamento(struct supermercato_t *s, int cliente, int num_oggetti)
{
    ...
    printf("CLIENTE %d IN SCEGLIE CASSA %d\n", cliente, cassa);
    s->coda_cassa[cassa]++;
    s->oggetti_cassa[cassa] += num_oggetti;

    pthread_cond_signal(&s->arrivo_cliente[cassa]);

    while (!s->cliente_allo_sportello[cassa])
    {
        pthread_cond_wait(&s->cassa[cassa], &s->mutex);
    }

    printf("CLIENTE %d SERVITO IN CASSA %d con %d items\n", cliente, cassa, num_oggetti);

    while (!s->cassa_finito[cassa])
    {
        pthread_cond_wait(&s->finito[cassa], &s->mutex);
    }

    s->oggetti_cassa[cassa] -= num_oggetti;
    s->cassa_finito[cassa] = 0;

    s->cliente_allo_sportello[cassa] = 0;
...
void cassiere_servo_cliente(struct supermercato_t *s, int cassa)
{
    ...
    while (!s->coda_cassa[cassa])
    {
        pthread_cond_wait(&s->arrivo_cliente[cassa], &s->mutex);
    }
    s->cliente_allo_sportello[cassa] = 1;
    s->coda_cassa[cassa]--;
    pthread_cond_signal(&s->cassa[cassa]);
...
void cassiere_fine_cliente(struct supermercato_t *s, int cassa)
{
    ...
    s->cassa_finito[cassa] = 1;
    pthread_cond_signal(&s->finito[cassa]);
